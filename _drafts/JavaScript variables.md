...
А в примере ниже мы получим ошибку, т.к. переменная `y` локальная, доступная только в блоке кода, ограниченный парой фигурных скобок.
...
В результате выполнения мы получим `ReferenceError: foo is not defined`. Почему так происходит, видь мы выше выяснили, что `var` - это глобальная переменная? Всё дело в области видимости.

Когда мы обращаемся к переменной `foo` за пределами функции `run()`, мы не можем получить её значение, потому, что она объявлена в области блока (block-scoped), заключена в фигурные скобки `{ ... }`. Тоже самое относится и к переменной `bar`. 
