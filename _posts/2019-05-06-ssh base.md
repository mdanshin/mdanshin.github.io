...
SSH implements a client-server model and is used to connect to remote computers and run commands. It also supports tunneling, which makes it similar to tunneling protocols (I wrote about tunneling earlier in my [VPN article](https://danshin.ms/vpn-base/)). SSH also provides `SSH file transfer (SFTP)` and `secure copy (SCP)`. To protect the channel, SSH uses `Public-key cryptography`.
...
There are several SSH implementations. I'm familiar only with [OpenSSH](https://www.openssh.com/) and used it mostly with CentOS, so the examples here are based on that. At the basic level described in this article, SSH works similarly across OSes.
...
I haven't fully explored the Windows OpenSSH client (where settings are stored, how to customize it, etc.), so for this example I'll use [KiTTY](http://www.9bis.net/kitty/) — it's a PuTTY fork. In practice, PuTTY and KiTTY behave very similarly for the purposes of this article.
...
We only care about the Base64-encoded part:
**"AAAAC3NzaC1lZDI1NTE5AAAAIDKEmQW2NUcy4waXWiKjI3hDa7YCvE49Eoekjdk2BLcl"**.
...
In the next article, [how to set up SSH public key authentication]({% post_url 2019-07-30-set up ssh with public-key %}), I explain how to generate your own public/private key pair, how to store it correctly, what to copy to the server, and which settings should be configured on the client and on the server.
</div>
