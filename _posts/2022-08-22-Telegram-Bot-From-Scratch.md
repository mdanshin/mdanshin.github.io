...
Например, вы моежет добавить бота в группу и используя [Telegram Bot API](https://core.telegram.org/bots/api#available-methods) отправлять от его имени сообщения в эту группу. Для этого нужно воспользоваться функцией `sendMessage`. Вот как это делается.
