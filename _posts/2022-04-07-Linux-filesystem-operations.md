...
|   |sudo fdisk -l|
|   |sudo parted -l|
|Как "разбиты" диски, сколько партиций, их тип и размер?|cat /proc/partitions|
|Сколько свободного места на дисках?|df -hT|
...
Преимещества:
1. При изменении порядка подключения дисков, название раздела может поменятся. Например, вместо sda стать sdb. Но UUID раздела станется прежнем, т.к. он хранится в суперблоке.
...
>UUID можно изменить командно tune2fs -U new_uuid /dev/sdaX
...
Есть несколько сценариев, когда нужно увеличить размер файловой системы. Вы увеличили размер виратуального диска. Восстановили образ системы на диск большего размера. Или добавили ещё один диск в систему.
...
запускаете fdisk, нажимаете `d`, чтобы удалить раздел. Вводите номер раздела. Затем нажимаете `n` и создаёте новый раздел, но уже большего размера. Затем выходите командой `w`.
