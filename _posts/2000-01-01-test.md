...
В заключение я бы рекомендовал перезапустить VM `reboot`. Во-первых это позволит загрузиться с нового ядра, которое было установлено во вребя обновления пакетов. А во-вторых убедиться в том, что squid автоматически запускается при старте системы.